document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.realizacjeData) {
    ...
    car.galleryImages.forEach(imgSrc => {
      galleryHTML += `
        <div class="masonry-item">
          <img src="${imgSrc.startsWith('/') ? '.' + imgSrc : imgSrc}" alt="Zdjęcie ${car.name}" loading="lazy" />
        </div>
      `;
    });
    ...
    galleryEl.innerHTML = '<p class="body-lg" style="color:var(--text-dim)">Nie udało się wczytać bazy danych (realizacje-data.js).</p>';
  }
});
